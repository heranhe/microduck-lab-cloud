...
`head_lead_s`; roadmap Track 4 item 12ae). Every knob ships OFF, so the first
test is that the shipped brain's head command is bit-identical with them at
their defaults; the rest drive one brain with synthetic frames and read the
...
def test_the_shipped_head_is_untouched_by_the_new_knobs_at_their_defaults():
    p = ChaseParams()
    assert (p.track_pitch, p.track_pitch_turn, p.look_hold_s, p.head_lead_s) == (False, 0.0, 0.0, 0.0)
    a = REGISTRY.make("chase", p=ChaseParams())
    b = REGISTRY.make("chase", p=ChaseParams(track_pitch=False, look_hold_s=0.0, head_lead_s=0.0))
...
def test_track_pitch_takes_over_where_the_gaze_declines():
    ...
    ship = _drive(REGISTRY.make("chase", p=ChaseParams(head_range=0.2)), frames, 0.6)
    ...
    assert 0.0 < last.head[1] <= 0.30                             # a pitch, capped
    ...
    a = _drive(REGISTRY.make("chase", p=ChaseParams()), frames, 0.6)
    c = _drive(REGISTRY.make("chase", p=ChaseParams(track_pitch=True)), frames, 0.6)
...
def test_track_pitch_defers_to_a_turn_in_place_unless_allowed():
    ...
    def turning(p):
        ...
        return turns[-1].head
    off = turning(ChaseParams(track_pitch=True))
...
def test_look_hold_keeps_the_head_on_the_coasting_track_past_predict_s():
    ...
    y_ship, _ = head_yaw_at(ChaseParams(), 1.6)
...
def test_head_lead_aims_ahead_of_a_rolling_ball():
    ...
    plain, lead = yaw(ChaseParams()), yaw(ChaseParams(head_lead_s=0.14))
